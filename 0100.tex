...
\keywords{backpropagation \and gradients \and neural networks \and softmax \and clipping.}
\end{abstract}
...
clipped gradients \cite{zhang2019gradient} \cite{smith2020generalization}. By rescaling the
...
self-distillation \cite{zhang2019your} \cite{wang2021memory}, identical student network
...
embedding based knowledge distillation \cite{aguilar2020knowledge} \cite{yao2018deep}
\cite{passalis2018unsupervised}. Distillation is applied to various tasks such as model
...
\cite{yuan2020revisiting}. Applications of logit suppression include: deep metric
learning \cite{zhai2018classification}, non-parametric neighborhood component analysis
...
\cite{liu2017sphereface} \cite{wang2018cosface}.
...
\url{ https://github.com/skalien/power-grad-transform }. Code is adapted from
\cite{rw2019timm}.}.
...
We examine issues that occur in non-BatchNormalized networks and we provide insight into
the effects of PGT on such networks. We use ResNet-18 \cite{he2016deep} as the
foundation model trained on ImageNet-1K \cite{russakovsky2015imagenet}. We found deeper
networks (such as ResNet-34 and ResNet-50 models) are impossible to train without Batch
Normalization because of increased depth. So in order to conduct our empirical
experiments, we solely focus on the ResNet-18 architecture. We designate different
layers with their corresponding layer indices. We provide a detailed layer-wise index
list of ResNet-18 in supplementary section \ref{sec:pgt_class_separation}.

Throughout the training process, we monitor variations in the the per-filter L2-norm of
each layer's weight tensor. Because a full plot of each layer's data would use an
inordinate amount of space, we focus on a few critical layers' plots. The supplementary
sections (\ref{sec:plots1} through \ref{sec:plots4}) contains full plots for all layer
for various settings. The norm of the weights of the $11^\textit{th}$ convolutional
layer is shown in Fig. \ref{fig:norm_plots}(a). Layer $11$ includes $256$ filters. Each
colour on the figure represents the iteration-wise evolution of a particular filter
throughout the course of training. The critical point to note is that some filters
achieve a norm of zero during training. We refer to this event as `Zeroing Out', and
occurs when a channel of a weight tensor gets fully filled with zeros. If all the
coefficients of the filter becomes zero, only then the L2 norm of that filter becomes
zero. If we observe zero filter norm for a filter, then that filter does not contribute
at all to determine the input-output relationship of a dataset, as the feature tensor it
produces is also an all-zero tensor. According to our findings, when a filter tensor
becomes zeroed out, it does not recover with further training. We observe both effects
\textbf{1)} zeroed out filter \textbf{2)} zeroed out feature.
